---
title: "Guía de Extensión y Creación de Plugins: idkfa"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de idkfa"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-idkfa-plugins)=
# Guía de Extensión y Plugins: idkfa

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`idkfa`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-idkfa-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`idkfa` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para idkfa`.
- **Punto de Entrada Oficial**: `idkfa.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-idkfa-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para idkfa."""
    name = "custom_rule"
...
Para que `idkfa` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."idkfa.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `idkfa` la reconozca:
...
(manual-idkfa-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `idkfa` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = idkfa.ejecutar_analisis(
...
(manual-idkfa-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `idkfa` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar idkfa
        run: pip install -e ./idkfa
        
      - name: Ejecutar Auditoría
        run: idkfa check src/ include/ --json > reporte.json
````

---

(manual-idkfa-plugins-ejercicios)=
